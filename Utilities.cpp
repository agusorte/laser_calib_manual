...
void Lu_method(MatrixXf x3d, MatrixXf x2d, MatrixXf &R,Vector3f &t, MatrixXf &xcam_, MatrixXf K,
  ...
  for (int j=0;j<cloud->points.size ();j++){
    ...
    //  b = img->imageData[img->widthStep * row+ col * 3];
     ...
     // g = img->imageData[img->widthStep * row+ col * 3 + 1];
      ...
      b=image.at<cv::Vec3b>(row,col)[0];

      g=image.at<cv::Vec3b>(row,col)[1];

      r=image.at<cv::Vec3b>(row,col)[2];
  ...
  //cloudRGB->points.resize(number_points);
  // PCL_INFO("image size RGB (%d, %d)\n",cloudRGB->points.size(),cloudRGB->points.size().height);
  PCL_INFO("Reading PCD RGB with %d points \n\n", cloudRGB->points.size());
  PCL_INFO("N points %d \n\n", number_points);
}
